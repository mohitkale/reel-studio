...
- [x] 22. Editable captions and local transcription. Depends on 21. Evidence: versioned caption tracks and cues persist through a safe populated-database migration; users can generate, import, select, enable, edit and export SRT/VTT tracks with visible timing provenance; spoken copy remains separate from scene headlines; optional shell-free whisper.cpp transcription reports actionable local setup; preview and export share caption inputs; 190 tests, production build, browser editing QA and real HyperFrames/Remotion H.264 renders pass. SHA: this commit.
